...
/*   Updated: 2025/05/21 20:56:47 by unovo-ru         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_var_type(va_list args, char c)
{
	...
	count = 0;
	if (c == 'c')
		count = count + ft_putchar_fd(va_arg(args, int));
	else if (c == 's')
		count = count + ft_putstr_fd(va_arg(args, char *));
	else if (c == 'p')
		count = count + ft_hexa(va_arg(args, void *));
	else if (c == 'd')
		count = count + ft_putnbr_fd(va_arg(args, int));
	else if (c == 'i')
		count = count + ft_putnbr_fd(va_arg(args, int));
	else if (c == 'u')
