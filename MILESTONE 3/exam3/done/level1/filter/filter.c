...
// Define el tamaño máximo del búfer. Añadimos +1 para el terminador nulo,
// asegurándonos de que podemos almacenar hasta 10 000 bytes de forma segura.
...
// Función ft_strncmp personalizada:
// Compara los primeros «n» caracteres de dos cadenas.
// Devuelve 1 si todos los «n» caracteres coinciden, 0 en caso contrario.
// Nota: Se trata de una función strncmp simplificada que solo comprueba la igualdad,
// no el orden lexicográfico, lo cual es suficiente para este problema.
int ft_strncmp(char *s1, char *s2, int n) {
    int i = 0;
    // Loop as long as 'i' is within 'n' bounds and characters at s1[i] and s2[i] are identical.
    // mientras i sea menor que n y los caracteres en s1[i] y s2[i] sean idénticos.
    while (i < n && s1[i] == s2[i]) {
        i++;
    }
    // If 'i' reached 'n', it means all 'n' characters matched.
    // Si i es igual que n, significa que todos los n caracteres coinciden.
    if (i == n) {
        return 1;
    } else {	  // si no, ha habido algun caracter diferente
...
int main(int ac, char **av) {
    ...
    // 1. Manejo de argumentos (MANEJO DE ERRORES IMPORTANTE) ---
    // El programa requiere exactamente un argumento (argc == 2).
    // El argumento no debe ser NULL y su longitud debe ser mayor que 0.
    ...
    // Asigna un búfer grande para almacenar toda la entrada de stdin.
    // MAX_INPUT_BUFFER_SIZE incluye espacio para el terminador nulo.
    ...
    // --- Manejo de errores de Malloc (MANEJO DE ERRORES IMPORTANTE) ---
    // Comprueba si malloc ha fallado. Si devuelve NULL, la asignación de memoria ha fallado.
    if (buff == NULL) {
